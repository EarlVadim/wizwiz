    ...
    if($orders->num_rows>0){
        while ($order = $orders->fetch_assoc()){
            ...
            if($expiryTime != null && $total != null){
                $send = "";
                if($expiryTime < $now_microdate + 86400000) $send = "День"; elseif($leftgb < 1) $send = "Гб";
                if($send != ""){  
                    $msg = "💡 Дорогой пользователь,
         От службы подписки $remark осталось всего ($send). \nВы можете продлить текущую услугу или купить новую услугу в разделе «Мои покупки».";
    ...
    if($orders->num_rows>0){
        while ($order = $orders->fetch_assoc()){
            ...
            if($send){  
        		...
        		if(!is_null($res)){
                    $msg = "💡 Дорогой пользователь,
     Срок действия подписки на службу $remark истек, и она была удалена из списка заказов. \nПожалуйста, приобретите новую услугу в магазине.";
                    sendMessage( $msg, null, null, $from_id);
