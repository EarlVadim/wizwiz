...
git clone https://github.com/EarlVadim/wizwiz.git /var/www/html/tgbot
sudo chown -R www-data:www-data /var/www/html/tgbot/
sudo chmod -R 755 /var/www/html/tgbot/
...
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/tgbot/settings/messagewizwiz.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/tgbot/settings/rewardReport.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/tgbot/settings/warnusers.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/tgbot/settings/gift2all.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "*/3 * * * * curl https://${DOMAIN_NAME}/tgbot/settings/tronChecker.php >/dev/null 2>&1") | sort - | uniq - | crontab -
...
wait
    ...
    if [[ $(mysql -u root -p$ROOT_PASSWORD -e "SHOW DATABASES LIKE 'wizwiz'") ]]; then
        ...
        file_path="/var/www/html/tgbot/baseInfo.php"
        ...
        # print file
        echo -e "<?php" >> /var/www/html/tgbot/baseInfo.php
        echo -e "error_reporting(0);" >> /var/www/html/tgbot/baseInfo.php
        echo -e "${ASAS}botToken = '${YOUR_BOT_TOKEN}';" >> /var/www/html/tgbot/baseInfo.php
        echo -e "${ASAS}dbUserName = '${dbuser}';" >> /var/www/html/tgbot/baseInfo.php
        echo -e "${ASAS}dbPassword = '${dbpass}';" >> /var/www/html/tgbot/baseInfo.php
        echo -e "${ASAS}dbName = '${dbname}';" >> /var/www/html/tgbot/baseInfo.php
        echo -e "${ASAS}botUrl = 'https://${YOUR_DOMAIN}/tgbot/';" >> /var/www/html/tgbot/baseInfo.php
        echo -e "${ASAS}admin = ${YOUR_CHAT_ID};" >> /var/www/html/tgbot/baseInfo.php
        echo -e "?>" >> /var/www/html/tgbot/baseInfo.php

        sleep 1

        curl -F "url=https://${YOUR_DOMAIN}/tgbot/bot.php" "https://api.telegram.org/bot${YOUR_BOT_TOKEN}/setWebhook"
        ...
        url="https://${YOUR_DOMAIN}/tgbot/createDB.php"
        curl $url
        
        sleep 1
        
        sudo rm -r /var/www/html/tgbot/webpanel
	    sudo rm -r /var/www/html/tgbot/install
	    sudo rm /var/www/html/tgbot/createDB.php
